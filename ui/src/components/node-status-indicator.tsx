...
export type NodeStatus = "scheduled" | "success" | "failed" | "running" | "skipped";
...
const StatusBorder = ({
  ...
  return (
    <>
      <div
        className={cn(
          "absolute -left-[2px] -top-[2px] h-[calc(100%+4px)] w-[calc(100%+4px)]  border-2",
          className,
        )}
      />
      {children}
...
export const NodeStatusIndicator = ({
  ...
  switch (status) {
    ...
    case "success":
      return (
        <StatusBorder className="border-emerald-500">{children}</StatusBorder>
      );
    case "failed":
